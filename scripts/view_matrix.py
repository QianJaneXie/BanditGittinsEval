...
DIR/<stem>.csv (no positional paths defaults to <repo>/outputs/matrices).
"""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "Omit when using --export-csv-dir to export from outputs/matrices by default.",
    )
    ...
    parser.add_argument(
        ...
        "If no paths are given, exports all *.npy from <repo>/outputs/matrices.",
    )
    ...
    if not paths_in and args.export_csv_dir is not None:
        paths_in = [str(root / "outputs" / "matrices")]
    elif not paths_in:
        parser.error("need at least one PATH, or use --export-csv-dir alone to export default outputs/matrices")
