#!/usr/bin/env python3
"""Download one matrix/subject from a W&B simple-regret sweep.

This is a standalone local script. It creates:
  - runs_summary.csv
  - runs_history.csv.gz
  - runs_stopping.csv
  - metadata.json
  - artifacts/  (only when --download-artifacts is used)

Example:
  python download_wandb_one_matrix_local.py \
    --entity EfficientLLMEval \
    --project GittinsBanditEval \
    --sweep-id 4dcyp8dx \
    --matrix-contains international_law \
    --states finished \
    --out-dir mmlu_small_international_law_finished
"""
...
def base_row(run: wandb.apis.public.Run) -> dict[str, Any]:
    ...
    return {
        ...
        "run_seed": cfg.get("run_seed", cfg.get("seed")),
...
def extract_stopping_from_trace(trace_path: Path) -> dict[str, Any]:
    ...
    except Exception as e:  # keep downloader running even if one artifact is broken
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--sweep-id", default=None, help="W&B sweep id, e.g. 4dcyp8dx")
    p.add_argument("--group", default=None, help="Optional W&B group filter, useful for rerun groups")
    p.add_argument("--matrix-contains", required=True, help="Substring in config['matrix'] or run name, e.g. international_law")
...
def main() -> int:
    args = parse_args()
    if not args.sweep_id and not args.group:
        raise SystemExit("Provide --sweep-id or --group")

    args.out_dir.mkdir(parents=True, exist_ok=True)
    keep_states = None if args.states.lower() == "all" else {s.strip() for s in args.states.split(",") if s.strip()}
    target = args.matrix_contains.lower()
        ...
        try:
            for run in runs:
                ...
                if n_runs_seen % 500 == 0:
                    print(f"scanned={n_runs_seen}, kept={n_runs_kept}, history_rows={n_history_rows}", flush=True)
                ...
                if target not in matrix and target not in name:
                    continue
    ...
    metadata = {
        ...
        "group": args.group,
        "matrix_contains": args.matrix_contains,
        "states": args.states,
