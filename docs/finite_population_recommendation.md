...
The LCB-aligned stopping rule compares the largest unfinished full-test-set Gittins index to the selected score `finite_mean - lambda * finite_std`. Fixed-budget runners record its first stopping time without truncating the trajectory, so it can be compared with natural stopping, which occurs when a completed arm wins the index comparison. LCB does not change the indices, sampling, or natural stopping; it changes the selected arm and the LCB-aligned threshold. The older crossing against the selected arm's unpenalized posterior mean remains logged only for compatibility with existing results.
